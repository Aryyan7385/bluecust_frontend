      ...
      <section className="pt-32 pb-24 px-6 relative overflow-hidden">
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <Button onClick={() => navigate('/register')} className="rounded-full bg-primary text-white font-bold px-8 py-6 text-lg shadow-xl hover:scale-105 transition-transform">Start Branding Now <ArrowRight className="ml-2"/></Button>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}>
            <img src="https://images.unsplash.com/photo-1638688569176-5b6db19f9d2a?auto=format&fit=crop&q=80&w=800" alt="Original Bottle Image" className="w-full rounded-3xl shadow-2xl animate-float" />
          </motion.div>
        </div>
      </section>

      {/* NEW: USP Section (Replaces Founder Section) */}
      <section className="py-24 px-6 bg-white/60 backdrop-blur-md">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-syne font-bold text-primary-900 uppercase tracking-tighter">Our USP</h2>
            <p className="text-slate-500 mt-2">Why ventures choose BlueCust for their brand endorsements</p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { icon: ShieldCheck, title: "Brand Endorsement", desc: "Turn every bottle into a marketing asset. Your customers see your name, building direct brand loyalty." },
              { icon: Banknote, title: "Standardized Pricing", desc: "Premium branding at a standard rate of ₹16 per bottle. Transparent costs, no hidden fees." },
              { icon: Zap, title: "Inventory Integration", desc: "We connect water suppliers and bottle manufacturers directly to your business dashboard." }
            ].map((item, i) => (
              <div key={i} className="p-8 bg-white rounded-3xl shadow-xl border border-blue-50 text-center hover:shadow-2xl transition-shadow">
          ...
          <div className="mt-20 text-center border-t border-blue-100 pt-10">
             <p className="text-xl font-syne font-bold text-primary-600 italic">Aryan Singh — Founder | Director, NextGenTech (Oct 2025)</p>
          </div>
        </div>
      </section>
    </div>
